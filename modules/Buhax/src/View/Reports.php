...
use Framelix\PhpSpreadsheet\SpreadsheetWrapper;
...
class Reports extends View
{
    ...
    /**
     * Get report sheet
     * @param \Framelix\Framelix\Date $dateFrom
     * @param \Framelix\Framelix\Date $dateTo
    ...
    public static function getReportSheet(
        ...
        $rangeLabel = $dateFrom->getHtmlString() . " - " . $dateTo->getHtmlString();
        $excelDataOutgoings = [
            [
                'nr' => Lang::get('__buhax_storable_outgoing_receiptnumber_label__'),
                'date' => Lang::get('__buhax_storable_outgoing_date_label__'),
                'comment' => Lang::get('__buhax_storable_outgoing_comment_label__'),
                'outgoingCategory' => Lang::get('__buhax_storable_outgoing_outgoingcategory_label__'),
                'summaryKeys' => Lang::get('__buhax_storable_systemvalue_outgoingcategory_summarykeys_label__'),
                'net' => Lang::get('__buhax_storable_outgoing_net_label__'),
                'netOperational' => Lang::get('__buhax_storable_outgoing_netoperational_label__'),
                'operationalSharePercent' => Lang::get(
                    '__buhax_storable_systemvalue_outgoingcategory_operationalsharepercent_label__'
                )
            ]
        ];
        $excelDataIncomes = [
            [
                'nr' => Lang::get('__buhax_storable_income_receiptnumber_label__'),
                'date' => Lang::get('__buhax_storable_income_date_label__'),
                'comment' => Lang::get('__buhax_storable_income_comment_label__'),
                'incomeCategory' => Lang::get('__buhax_storable_income_incomecategory_label__'),
                'summaryKeys' => Lang::get('__buhax_storable_systemvalue_incomecategory_summarykeys_label__'),
                'net' => Lang::get('__buhax_storable_income_net_label__')
            ]
        ];
        $excelDataProfitLoss = [
            [
                '',
                Lang::get('__buhax_view_outgoings__'),
                Lang::get('__buhax_view_incomes__'),
                Lang::get('__buhax_view_report_profitloss__')
            ]
        ];
        ...
        foreach ($outgoings as $outgoing) {
            $summaryKeys = $outgoing->outgoingCategory->summaryKeys;
            $excelDataOutgoings[] = [
        ...
        foreach ($incomes as $income) {
            $summaryKeys = $income->incomeCategory->summaryKeys;
            $excelDataIncomes[] = [
        ...
        $excelData = [
            Lang::get('__buhax_view_outgoings__') => $excelDataOutgoings,
            Lang::get('__buhax_view_incomes__') => $excelDataIncomes,
            Lang::get('__buhax_view_backend_systemvalue_summarykey__') => $excelDataSummaryKeys,
            Lang::get('__buhax_view_report_profitloss__') => $excelDataProfitLoss
        ];
    ...
    public function showContent(): void
    {
        ...
        if ($rangeId !== null && isset($dateRanges[$rangeId])) {
            ...
            if ($dateRange['type'] === 'custom') {
                $form = $this->getForm();
                $form->addSubmitButton('report-charts', '__buhax_view_reports_generate__', 'leaderboard');
                $form->show();
            }
            ...
            if (isset($dateFrom) && isset($dateTo)) {
                ...
                $cards = [
                    'profitloss' => [
                        'label' => '__buhax_view_report_profitloss__',
                        'now' => 0.0,
                        'prev' => null,
                    ],
                    'incomes' => [
                        'label' => '__buhax_view_incomes__',
                        'now' => 0.0,
                        'prev' => null,
                    ],
                    'outgoings' => [
                        'label' => '__buhax_view_outgoings__',
                        'now' => 0.0,
                        'prev' => null,
                    ]
                ];
                ...
                ?>
                <a href="<?= $this->getSelfUrl()->setParameter('dateFrom', $dateFrom)->setParameter(
                    'dateTo',
                    $dateTo
                )->setParameter('excel', 1) ?>"
                   class="framelix-button framelix-button-success"><?= Lang::get(
                        '__buhax_view_reports_generate_excel__'
                    ) ?></a>
                    ...
                    foreach ($cards as $rowType => $row) {
                        ...
                        if (isset($dateFromPrev) && isset($dateToPrev)) {
                            $comparedTitle = Lang::get(
                                '__buhax_view_report_compared_to__',
                            ...
                            switch ($rowType) {
                                case 'profitloss':
                                    $label = '__buhax_view_report_profit__';
                                    if ($diff > 0) {
                                        $diffType = 'better';
                                    }
                                    if ($row['now'] < 0) {
                                        $label = '__buhax_view_report_loss__';
                                    }
                                    break;
                                case 'incomes':
                                    if ($diff > 0) {
                            ...
                            if ($rowType !== 'profitloss') {
                                $diffLabel .= Config::get(
                                        'moneyUnit'
                                    ) . " <span class='report-amount-diff-info'>(" . NumberUtils::format(
                                        $row['now'] != 0 ? ceil((100 / abs($row['now']) * $diff)) : 0,
                                        plusSign: true
                                    ) . "%)</span>";
                            }
                            $diffLabelTooltip = '__buhax_view_report_' . $diffType . '_' . $rowType . '__';
                            if ($diffType === 'better') {
                                $icon = 'north';
                            ...
                            <div class="report-amount">
                                ...
                                <div class="report-amount-value">
                                    ...
                                    if (isset($row['prev'])) {
                                        ...
                                        ?>
                                        <div class="report-amount-diff-prev" title="<?= $comparedTitle ?>">
                                            <?= $number . Config::get('moneyUnit') ?>
                                        </div>
                                        <?php
                                    }
                                    ?>
                                    ...
                                    echo $number . Config::get('moneyUnit');
                                    if ($rowType === 'profitloss') {
                                        echo '</span>';
                                    }
                                    ?>
            ...
            foreach ($dateRanges as $i => $row) {
                ...
                $tabs->addTab($i, "__buhax_view_reports_" . $row['label'] . "__", new self(), ['index' => $i]);
            }
            $tabs->show();
        }
        ?>
    ...
    private function getForm(): Form
    {
        ...
        $field = new Date();
        $field->name = "dateFrom";
        $field->label = "__buhax_view_reports_datefrom__";
        $field->required = true;
        ...
        $field = new Date();
        $field->name = "dateTo";
        $field->label = "__buhax_view_reports_dateto__";
        $field->required = true;
